...
Requests go through **[Webshare](https://webshare.io) residential proxies** using [`impit`](https://github.com/nicbarker/impit) for browser-like TLS fingerprinting. Rate limiting (150 req/hr token bucket), session rotation, and retry logic keep it from getting blocked.
...
```env
# Required — Webshare residential proxy credentials
...
POST /scrape/instagram { username: "natgeo", maxPosts: 50 }
  ...
  │    └─ impit (TLS fingerprint) + Webshare residential proxy
...
| Platform  | Proxy type           | On 403 block   | Cooldown                      |
| --------- | -------------------- | -------------- | ----------------------------- |
| Instagram | Webshare residential | Rotate session | 15-min cooldown after 3 fails |
| TikTok    | Webshare residential | Rotate session | —                             |
...
| ------------------- | ------------- | ---------------------------------- |
| `WEBSHARE_USERNAME` | Docker deploy | Webshare proxy username            |
| `WEBSHARE_PASSWORD` | Docker deploy | Webshare proxy password            |
...
| :------------------- | :-------------------- | :----------------------------------------- | :---- |
| Webshare Proxy       | ✅ Active             | Residential proxies for IG/TT              | 1     |
